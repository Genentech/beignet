...
def test_multiply_physicists_hermite_series():
    ...
    for j in range(5):
        ...
        val1 = beignet.polynomial.evaluate_1d_physicists_hermite_series(x, pol1)

        for k in range(5):
            pol2 = torch.tensor([0] * k + [1])
            val2 = beignet.polynomial.evaluate_1d_physicists_hermite_series(x, pol2)
            pol3 = beignet.polynomial.multiply_physicists_hermite_series(pol1, pol2)
            val3 = beignet.polynomial.evaluate_1d_physicists_hermite_series(x, pol3)

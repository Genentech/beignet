...
import beignet.polynomial._chebfit
...
def trim(x):
    return beignet.polynomial._chebtrim.chebtrim(x, tol=1e-6)
    ...
    def test_chebsub(self):
        for i in range(5):
            for j in range(5):
                ...
                tgt[i] += 1
                tgt[j] -= 1
                res = beignet.polynomial._chebsub.chebsub([0] * i + [1], [0] * j + [1])
                numpy.testing.assert_equal(trim(res), trim(tgt), err_msg=msg)
    ...
    def test_chebmul(self):
        for i in range(5):
            for j in range(5):
                ...
                tgt[i + j] += 0.5
                tgt[abs(i - j)] += 0.5
                res = beignet.polynomial._chebmul.chebmul([0] * i + [1], [0] * j + [1])
                numpy.testing.assert_equal(trim(res), trim(tgt), err_msg=msg)
        ...
        for i in range(5):
            for j in range(5):
                ...
                tgt = functools.reduce(
                    beignet.polynomial._chebmul.chebmul, [c] * j, numpy.array([1])
                )
...
class TestEvaluation:
    ...
    def test_chebval(self):
        # check empty input
        numpy.testing.assert_equal(beignet.polynomial._chebval.chebval([], [1]).size, 0)
        ...
        for i in range(10):
            msg = f"At i={i}"
            tgt = y[i]
            res = beignet.polynomial._chebval.chebval(x, [0] * i + [1])
        ...
        for i in range(3):
            ...
            numpy.testing.assert_equal(
                beignet.polynomial._chebval.chebval(x, [1]).shape, dims
            )
            numpy.testing.assert_equal(
                beignet.polynomial._chebval.chebval(x, [1, 0]).shape, dims
            )
            numpy.testing.assert_equal(
                beignet.polynomial._chebval.chebval(x, [1, 0, 0]).shape, dims
            )
    ...
    def test_chebval2d(self):
        ...
        numpy.testing.assert_raises(
            ValueError, beignet.polynomial._chebval2d.chebval2d, x1, x2[:2], self.c2d
        )

        # test values
        tgt = y1 * y2
        res = beignet.polynomial._chebval2d.chebval2d(x1, x2, self.c2d)
        numpy.testing.assert_almost_equal(res, tgt)

        # test shape
        z = numpy.ones((2, 3))
        res = beignet.polynomial._chebval2d.chebval2d(z, z, self.c2d)
    ...
    def test_chebval3d(self):
        ...
        numpy.testing.assert_raises(
            ValueError,
            beignet.polynomial._chebval3d.chebval3d,
            x1,
            x2,
            x3[:2],
            self.c3d,
        ...
        # test values
        tgt = y1 * y2 * y3
        res = beignet.polynomial._chebval3d.chebval3d(x1, x2, x3, self.c3d)
        numpy.testing.assert_almost_equal(res, tgt)

        # test shape
        z = numpy.ones((2, 3))
        res = beignet.polynomial._chebval3d.chebval3d(z, z, z, self.c3d)
        numpy.testing.assert_(res.shape == (2, 3))
...
class TestDerivative:
    def test_chebder(self):
        ...
        for i in range(5):
            tgt = [0] * i + [1]
            res = beignet.polynomial._chebder.chebder(tgt, m=0)
        ...
        for i in range(5):
            for j in range(2, 5):
                tgt = [0] * i + [1]
                res = beignet.polynomial._chebder.chebder(
        ...
        for i in range(5):
            for j in range(2, 5):
                tgt = [0] * i + [1]
                res = beignet.polynomial._chebder.chebder(
                    beignet.polynomial._chebint.chebint(tgt, m=j, scl=2), m=j, scl=0.5
                )
                numpy.testing.assert_almost_equal(trim(res), trim(tgt))
    ...
    def test_chebder_axis(self):
        ...
        tgt = numpy.vstack([beignet.polynomial._chebder.chebder(c) for c in c2d.T]).T
        res = beignet.polynomial._chebder.chebder(c2d, axis=0)
        numpy.testing.assert_almost_equal(res, tgt)

        tgt = numpy.vstack([beignet.polynomial._chebder.chebder(c) for c in c2d])
        res = beignet.polynomial._chebder.chebder(c2d, axis=1)
        numpy.testing.assert_almost_equal(res, tgt)
...
class TestVander:
    ...
    def test_chebvander2d(self):
        ...
        tgt = beignet.polynomial._chebval2d.chebval2d(x1, x2, c)
    ...
    def test_chebvander3d(self):
        ...
        tgt = beignet.polynomial._chebval3d.chebval3d(x1, x2, x3, c)
        res = numpy.dot(van, c.flat)
        numpy.testing.assert_almost_equal(res, tgt)
...
class TestFitting:
    def test_chebfit(self):
        ...
        # Test exceptions
        numpy.testing.assert_raises(
            ValueError, beignet.polynomial._chebfit.chebfit, [1], [1], -1
        )
        numpy.testing.assert_raises(
            TypeError, beignet.polynomial._chebfit.chebfit, [[1]], [1], 0
        )
        numpy.testing.assert_raises(
            TypeError, beignet.polynomial._chebfit.chebfit, [], [1], 0
        )
        numpy.testing.assert_raises(
            TypeError, beignet.polynomial._chebfit.chebfit, [1], [[[1]]], 0
        )
        numpy.testing.assert_raises(
            TypeError, beignet.polynomial._chebfit.chebfit, [1, 2], [1], 0
        )
        numpy.testing.assert_raises(
            TypeError, beignet.polynomial._chebfit.chebfit, [1], [1, 2], 0
        )
        numpy.testing.assert_raises(
            TypeError, beignet.polynomial._chebfit.chebfit, [1], [1], 0, w=[[1]]
        )
        numpy.testing.assert_raises(
            TypeError, beignet.polynomial._chebfit.chebfit, [1], [1], 0, w=[1, 1]
        )
        numpy.testing.assert_raises(
            ValueError,
            beignet.polynomial._chebfit.chebfit,
            [1],
            [1],
            [
                -1,
        ...
        numpy.testing.assert_raises(
            ValueError, beignet.polynomial._chebfit.chebfit, [1], [1], [2, -1, 6]
        )
        numpy.testing.assert_raises(
            TypeError, beignet.polynomial._chebfit.chebfit, [1], [1], []
        )
        ...
        y = f(x)
        #
        coef3 = beignet.polynomial._chebfit.chebfit(x, y, 3)
        numpy.testing.assert_equal(len(coef3), 4)
        numpy.testing.assert_almost_equal(
            beignet.polynomial._chebval.chebval(x, coef3), y
        )
        coef3 = beignet.polynomial._chebfit.chebfit(x, y, [0, 1, 2, 3])
        numpy.testing.assert_equal(len(coef3), 4)
        numpy.testing.assert_almost_equal(
            beignet.polynomial._chebval.chebval(x, coef3), y
        )
        #
        coef4 = beignet.polynomial._chebfit.chebfit(x, y, 4)
        numpy.testing.assert_equal(len(coef4), 5)
        numpy.testing.assert_almost_equal(
            beignet.polynomial._chebval.chebval(x, coef4), y
        )
        coef4 = beignet.polynomial._chebfit.chebfit(x, y, [0, 1, 2, 3, 4])
        numpy.testing.assert_equal(len(coef4), 5)
        numpy.testing.assert_almost_equal(
            beignet.polynomial._chebval.chebval(x, coef4), y
        )
        # check things still work if deg is not in strict increasing
        coef4 = beignet.polynomial._chebfit.chebfit(x, y, [2, 3, 4, 1, 0])
        numpy.testing.assert_equal(len(coef4), 5)
        numpy.testing.assert_almost_equal(
            beignet.polynomial._chebval.chebval(x, coef4), y
        )
        #
        coef2d = beignet.polynomial._chebfit.chebfit(x, numpy.array([y, y]).T, 3)
        numpy.testing.assert_almost_equal(coef2d, numpy.array([coef3, coef3]).T)
        coef2d = beignet.polynomial._chebfit.chebfit(
        ...
        w[1::2] = 1
        y[0::2] = 0
        wcoef3 = beignet.polynomial._chebfit.chebfit(x, yw, 3, w=w)
        numpy.testing.assert_almost_equal(wcoef3, coef3)
        wcoef3 = beignet.polynomial._chebfit.chebfit(x, yw, [0, 1, 2, 3], w=w)
        numpy.testing.assert_almost_equal(wcoef3, coef3)
        #
        wcoef2d = beignet.polynomial._chebfit.chebfit(
        ...
        x = [1, 1j, -1, -1j]
        numpy.testing.assert_almost_equal(
            beignet.polynomial._chebfit.chebfit(x, x, 1), [0, 1]
        )
        numpy.testing.assert_almost_equal(
            beignet.polynomial._chebfit.chebfit(x, x, [0, 1]), [0, 1]
        )
        # test fitting only even polynomials
        x = numpy.linspace(-1, 1)
        y = f2(x)
        coef1 = beignet.polynomial._chebfit.chebfit(x, y, 4)
        numpy.testing.assert_almost_equal(
            beignet.polynomial._chebval.chebval(x, coef1), y
        )
        coef2 = beignet.polynomial._chebfit.chebfit(x, y, [0, 2, 4])
        numpy.testing.assert_almost_equal(
            beignet.polynomial._chebval.chebval(x, coef2), y
        )
...
class TestMisc:
    def test_chebfromroots(self):
        res = beignet.polynomial._chebfromroots.chebfromroots([])
        numpy.testing.assert_almost_equal(trim(res), [1])
        for i in range(1, 5):
            roots = numpy.cos(numpy.linspace(-numpy.pi, 0, 2 * i + 1)[1::2])
            tgt = [0] * i + [1]
            res = beignet.polynomial._chebfromroots.chebfromroots(roots) * 2 ** (i - 1)
            numpy.testing.assert_almost_equal(trim(res), trim(tgt))

    def test_chebroots(self):
        numpy.testing.assert_almost_equal(
            beignet.polynomial._chebroots.chebroots([1]), []
        )
        numpy.testing.assert_almost_equal(
        ...
        for i in range(2, 5):
            ...
            res = beignet.polynomial._chebroots.chebroots(
    ...
    def test_chebtrim(self):
        ...
        # Test exceptions
        numpy.testing.assert_raises(
            ValueError, beignet.polynomial._chebtrim.chebtrim, coef, -1
        )

        # Test results
        numpy.testing.assert_equal(
            beignet.polynomial._chebtrim.chebtrim(coef), coef[:-1]
        )
        numpy.testing.assert_equal(
            beignet.polynomial._chebtrim.chebtrim(coef, 1), coef[:-3]
        )
        numpy.testing.assert_equal(beignet.polynomial._chebtrim.chebtrim(coef, 2), [0])
    ...
    def test_cheb2poly(self):
        for i in range(10):
            numpy.testing.assert_almost_equal(
                beignet.polynomial._cheb2poly.cheb2poly([0] * i + [1]), Tlist[i]
            )

    def test_poly2cheb(self):
        for i in range(10):
            numpy.testing.assert_almost_equal(
                beignet.polynomial._poly2cheb.poly2cheb(Tlist[i]), [0] * i + [1]
            )

...
from ..functional.statistics import jonckheere_terpstra_test_power


class JonckheereTerpstraTestPower(Metric):
    r"""
    Compute statistical power for Jonckheere-Terpstra test.

    This metric calculates the statistical power for Jonckheere-Terpstra test,
    which is a non-parametric test for ordered alternatives. It follows
    ...
    --------
    >>> import torch
    >>> from beignet.metrics import JonckheereTerpstraTestPower
    >>> metric = JonckheereTerpstraTestPower()
    ...
    def update(self, effect_size: Tensor, sample_size: Tensor, groups: Tensor) -> None:
        ...
        ----------
        effect_size : Tensor
            Effect size measure for ordered alternatives.
        sample_size : Tensor
            Total sample size.
        groups : Tensor
            Number of ordered groups.
        """
    ...
    def compute(self) -> Tensor:
        ...
        return jonckheere_terpstra_test_power(
            effect_size,
            sample_size,
            groups,
            alpha=self.alpha,
    ...
    def plot(
        ...
        **kwargs: Any,
        ...
        """Plot Jonckheere-Terpstra test power visualization."""
        try:
        ...
        if plot_type == "power_curve":
            ...
            for es in effect_sizes:
                power = jonckheere_terpstra_test_power(
                    torch.tensor(es),
            ...
            if title is None:
                title = f"Jonckheere-Terpstra Test Power (α={self.alpha}, N={current_n}, Groups={current_groups})"
        else:

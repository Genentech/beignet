...
from ..functional.statistics import jonckheere_terpstra_test_sample_size


class JonckheereTerpstraTestSampleSize(Metric):
    r"""
    Compute required sample size for Jonckheere-Terpstra test.

    This metric calculates the required sample size for Jonckheere-Terpstra test
    ...
    --------
    >>> import torch
    >>> from beignet.metrics import JonckheereTerpstraTestSampleSize
    >>> metric = JonckheereTerpstraTestSampleSize()
    ...
    def compute(self) -> Tensor:
        ...
        return jonckheere_terpstra_test_sample_size(
            effect_size,
            groups,
            power=self.power,
            alpha=self.alpha,
    ...
    def plot(
        ...
        **kwargs: Any,
        ...
        """Plot Jonckheere-Terpstra test sample size visualization."""
        try:
        ...
        if plot_type == "sample_size_curve":
            ...
            for es in effect_sizes:
                n = jonckheere_terpstra_test_sample_size(
                    torch.tensor(es),
            ...
            if title is None:
                title = f"Jonckheere-Terpstra Test Sample Size (Power={self.power}, α={self.alpha}, Groups={current_groups})"
        else:

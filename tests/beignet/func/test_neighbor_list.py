...
#
#     box_size = (
#         torch.tensor([9.0, 4.0, 7.25], dtype=torch.float32) if dim == 3 else
#         torch.tensor([9.0, 4.25], dtype=torch.float32))
...
#     dR_exact = torch.where(dR_exact < cutoff, dR_exact, torch.tensor(0, dtype=torch.float32)) * mask_exact
#
...
#         dR_exact_row = torch.tensor(dR_exact_row[dR_exact_row > 0.], dtype=dtype)
#

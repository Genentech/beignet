...
def test_chi_squared_goodness_of_fit_power(
    ...
    """Test ChiSquaredGoodnessOfFitPower TorchMetrics class."""
    # Initialize the metric
    metric = beignet.metrics.statistics.ChiSquaredGoodnessOfFitPower(alpha=alpha)
    ...
    if dtype == torch.float32:
        ...
        metric_64 = beignet.metrics.statistics.ChiSquaredGoodnessOfFitPower(alpha=alpha)
    ...
    metric_small = beignet.metrics.statistics.ChiSquaredGoodnessOfFitPower(alpha=alpha)
    ...
    metric_large = beignet.metrics.statistics.ChiSquaredGoodnessOfFitPower(alpha=alpha)
    ...
    assert "ChiSquaredGoodnessOfFitPower" in repr_str
    assert f"alpha={alpha}" in repr_str

    # Test device consistency
    if torch.cuda.is_available():
        device = torch.device("cuda")
        metric_cuda = beignet.metrics.statistics.ChiSquaredGoodnessOfFitPower(
            alpha=alpha,
        ).to(device)
    ...
    metric_grad = beignet.metrics.statistics.ChiSquaredGoodnessOfFitPower(alpha=alpha)

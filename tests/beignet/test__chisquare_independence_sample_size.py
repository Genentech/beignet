...
from beignet._chisquare_independence_power import chisquare_independence_power
from beignet._chisquare_independence_sample_size import (
...
def test_chisquare_independence_sample_size(
    ...
    sample_size = chisquare_independence_sample_size(
    ...
    sample_size_low = chisquare_independence_sample_size(
        effect_size, rows, cols, power=0.6, alpha=alpha
    )
    sample_size_high = chisquare_independence_sample_size(
    ...
    sample_size_strict = chisquare_independence_sample_size(
        effect_size, rows, cols, power=power, alpha=0.01
    )
    sample_size_lenient = chisquare_independence_sample_size(
    ...
    sample_size_small = chisquare_independence_sample_size(
        small_effect, rows, cols, power=power, alpha=alpha
    )
    sample_size_large = chisquare_independence_sample_size(
    ...
    sample_size_small_table = chisquare_independence_sample_size(
        effect_size, small_rows, small_cols, power=power, alpha=alpha
    )
    sample_size_large_table = chisquare_independence_sample_size(
    ...
    compiled_func = torch.compile(chisquare_independence_sample_size, fullgraph=True)
    sample_size_compiled = compiled_func(
        effect_size.detach(), rows.detach(), cols.detach(), power=power, alpha=alpha
    )
    sample_size_regular = chisquare_independence_sample_size(
        effect_size.detach(), rows.detach(), cols.detach(), power=power, alpha=alpha
    )
    ...
    result = chisquare_independence_sample_size(
        effect_size.detach(),
        rows.detach(),
        cols.detach(),
        power=power,
    ...
    large_sample = chisquare_independence_sample_size(
    ...
    small_sample = chisquare_independence_sample_size(
...
def test_chisquare_independence_sample_size_special_cases() -> None:
    ...
    sample_size_2x2 = chisquare_independence_sample_size(
        effect_size, rows, cols, power=power, alpha=alpha
    )
    ...
    # Test larger tables
    sample_size_3x3 = chisquare_independence_sample_size(
        effect_size, torch.tensor(3), torch.tensor(3), power=power, alpha=alpha
    )
    sample_size_4x4 = chisquare_independence_sample_size(
    ...
    sample_size_2x5 = chisquare_independence_sample_size(
        effect_size, torch.tensor(2), torch.tensor(5), power=power, alpha=alpha
    )
    sample_size_5x2 = chisquare_independence_sample_size(
    ...
    sample_size_high_power = chisquare_independence_sample_size(

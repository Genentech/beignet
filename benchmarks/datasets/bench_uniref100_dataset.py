...
from beignet.datasets import UniRef100Dataset


class BenchUniRef100Dataset:
    params = [
        [10, 100, 1000],
    ]
    ...
    def setup(self, batch_size):
        self.dataset = UniRef100Dataset(
            root=os.getenv("BEIGNET_BENCHMARKS_DATASET_ROOT", "/tmp") + "/uniref100",
            download=True,
            transform=None,
    ...
    def time___init__(self, batch_size):
        UniRef100Dataset(
            root=os.getenv("BEIGNET_BENCHMARKS_DATASET_ROOT", ".") + "/uniref100",
            download=True,
            transform=None,
            target_transform=None,
        )

...
from ..functional.statistics import paired_t_test_power


class PairedTTestPower(Metric):
    r"""
    Compute statistical power for paired t-test.

    This metric calculates the statistical power for paired t-test, which is
    the probability of correctly rejecting a false null hypothesis when comparing
    paired observations. It follows TorchMetrics conventions for stateful metric
    computation.
    ...
    --------
    >>> import torch
    >>> from beignet.metrics import PairedTTestPower
    >>> metric = PairedTTestPower()
    ...
    def plot(
        ...
        **kwargs: Any,
        ...
        """Plot paired t-test power visualization.
        ...
        **kwargs : dict
        ...
        if plot_type == "power_curve":
            ...
            if title is None:
                title = f"Paired T-Test Power Analysis (α={self.alpha}, N={current_n})"
        else:

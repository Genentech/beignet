...
from ..functional.statistics import paired_t_test_sample_size


class PairedTTestSampleSize(Metric):
    r"""
    Compute required sample size for paired t-test.

    This metric calculates the required sample size for paired t-test to achieve
    ...
    --------
    >>> import torch
    >>> from beignet.metrics import PairedTTestSampleSize
    >>> metric = PairedTTestSampleSize()
    ...
    def compute(self) -> Tensor:
        ...
        return paired_t_test_sample_size(
            effect_size,
            power=self.power,
            alpha=self.alpha,
    ...
    def plot(
        ...
        **kwargs: Any,
        ...
        """Plot paired t-test sample size visualization.
        ...
        **kwargs : dict
        ...
        if plot_type == "sample_size_curve":
            ...
            if title is None:
                title = (
                    f"Paired T-Test Sample Size (Power={self.power}, α={self.alpha})"
                )
        else:

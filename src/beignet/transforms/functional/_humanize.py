...
def substitute_chars(str1, str2, mask):
    return "".join(s2 if m else s1 for s1, s2, m in zip(str1, str2, mask, strict=False))
...
def humanize(
    ...
    df : pd.DataFrame
        ...
        Columns are 'fv_heavy', 'fv_light', 'fv_heavy_likelihood',
        'fv_light_likelihood'.
    """
    # Load the model
    ...
    if naturalness:  # slow
        ...
        fv_heavy_data["fv_heavy_naturalness"] = (
            fv_heavy_humanized_naturalness.cpu().numpy()
        )
        fv_light_data["fv_light_naturalness"] = (

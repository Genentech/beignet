...
from beignet.datasets import ToxCastDataset


class BenchToxCastDataset:
    params = [
        [10, 100, 1000],
    ]
    ...
    def setup(self, batch_size):
        self.dataset = ToxCastDataset(
            root=os.getenv("BEIGNET_BENCHMARKS_DATASET_ROOT", "/tmp") + "/toxcast",
            download=True,
            transform=None,
    ...
    def time___init__(self, batch_size):
        ToxCastDataset(
            root=os.getenv("BEIGNET_BENCHMARKS_DATASET_ROOT", ".") + "/toxcast",
            download=True,
            transform=None,
            target_transform=None,
        )

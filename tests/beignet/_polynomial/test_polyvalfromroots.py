...
def test_polyvalfromroots():
    ...
    for i in range(1, 5):
        numpy.testing.assert_almost_equal(
            beignet.polynomial._polyvalfromroots.polyvalfromroots(x, [0] * i), y[i]
        )
    tgt = x * (x - 1) * (x + 1)
    numpy.testing.assert_almost_equal(
        beignet.polynomial._polyvalfromroots.polyvalfromroots(x, [-1, 0, 1]), tgt
    )
    ...
    for i in range(3):
        ...
        x = numpy.zeros(dims)
        torch.testing.assert_close(
            beignet.polynomial._polyvalfromroots.polyvalfromroots(x, [1]).shape, dims
        )
        torch.testing.assert_close(
            beignet.polynomial._polyvalfromroots.polyvalfromroots(x, [1, 0]).shape, dims
        )
        torch.testing.assert_close(
            beignet.polynomial._polyvalfromroots.polyvalfromroots(x, [1, 0, 0]).shape,
            dims,
        )
    ...
    numpy.testing.assert_almost_equal(
        ...
        beignet.polynomial._polyvalfromroots.polyvalfromroots(
            x, beignet.polynomial._polyroots.polyroots(ptest)
        ),
    )
    ...
    for ii in range(tgt.size):
        tgt[ii] = beignet.polynomial._polyvalfromroots.polyvalfromroots(x[ii], r[:, ii])
    torch.testing.assert_close(
        beignet.polynomial._polyvalfromroots.polyvalfromroots(x, r, tensor=False), tgt
    )
    ...
    torch.testing.assert_close(
        beignet.polynomial._polyvalfromroots.polyvalfromroots(x, r, tensor=True), tgt
    )

"""Independent z-test sample size metric."""
...
from beignet._independent_z_test_sample_size import independent_z_test_sample_size


class IndependentZTestSampleSize(Metric):
    """Computes required sample size for independent samples z-tests with known variances.
    ...
    ratio : Tensor, optional
        ...
        Group 2 sample size = sample_size1 * ratio.
    ...
    Examples
    --------
    >>> metric = IndependentZTestSampleSize()
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.add_state("ratio", default=torch.tensor(1.0), dist_reduce_fx="mean")
        self.add_state("total", default=torch.tensor(0), dist_reduce_fx="sum")

    def update(self, effect_size: Tensor, ratio: Tensor | None = None) -> None:
        ...
        if self.total == 0:
            ...
            self.ratio = (self.ratio * self.total + ratio) / (self.total + 1)
        self.total += 1
    ...
    def plot(
        ...
        **kwargs: Any,
        ...
        dep_var : str, default="effect_size"
            The variable to plot on x-axis. Either "effect_size", "power", or "ratio".
        ...
        **kwargs : dict
        ...
        >>> fig = metric.plot(dep_var="ratio", effect_size=[0.2, 0.5, 0.8])
        """
        try:
        ...
        if title is None:
            title = (
                f"Sample Size Analysis: Independent Samples Z-Test ({self.alternative})"
            )
        ax.set_title(title)

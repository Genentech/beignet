...
def test_friedman_test_sample_size(batch_size, dtype):
    """Test friedman_test_sample_size functional wrapper."""
    ...
    try:
        result_functional = (
            beignet.metrics.functional.statistics.friedman_test_sample_size(test_input)
        )
        result_direct = beignet.statistics.friedman_test_sample_size(test_input)
        ...
        assert hasattr(beignet.statistics, "friedman_test_sample_size")
        assert callable(beignet.metrics.functional.statistics.friedman_test_sample_size)
        assert callable(beignet.statistics.friedman_test_sample_size)

...
class TriangleAttentionEndingNode(nn.Module):
    ...
    def forward(self, z_ij: Tensor) -> Tensor:
        ...
        # Step 5: Attention computation (ending node version)
        # α^h_ijk = softmax_k(1/√c * q^h_ij^T k^h_kj + b^h_ij)
        # For ending node attention, we attend from i to k for each edge (i,j)
        # The key difference: we use k^h_kj instead of k^h_jk
        ...
        # We need k_kj: key for edge (k,j) - this means we need k[k,j] from k[i,j]

        # For each position (i,j), we want to attend to all k positions using k[k,j]
        # This means: for edge (i,j), use query q[i,j] and attend over k[:,j] (all edges ending at j)

        # Compute attention scores using einsum
        # q_ij: (..., i, j, h, d), k_ij: (..., k, j, h, d) -> (..., i, j, h, k)
        attn_scores = torch.einsum("...ijhd,...kjhd->...ijhk", q_ij, k_ij) * self.scale
        ...
        # Step 6: Weighted sum of values (ending node version)
        # o^h_ij = g^h_ij ⊙ ∑_k α^h_ijk v^h_kj
        # The key difference: we use v^h_kj instead of v^h_jk

        # Compute weighted sum using einsum
        # alpha_ijk: (..., i, j, h, k), v_ij: (..., k, j, h, d) -> (..., i, j, h, d)
        o_ij = torch.einsum("...ijhk,...kjhd->...ijhd", alpha_ijk, v_ij)

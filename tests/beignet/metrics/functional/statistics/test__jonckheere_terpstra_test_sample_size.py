...
def test_jonckheere_terpstra_test_sample_size(batch_size, dtype):
    """Test jonckheere_terpstra_test_sample_size functional wrapper."""
    ...
    try:
        result_functional = (
            beignet.metrics.functional.statistics.jonckheere_terpstra_test_sample_size(
                test_input,
            )
        )
        result_direct = beignet.statistics.jonckheere_terpstra_test_sample_size(
            test_input,
        )
        ...
        assert hasattr(beignet.statistics, "jonckheere_terpstra_test_sample_size")
        assert callable(
            beignet.metrics.functional.statistics.jonckheere_terpstra_test_sample_size,
        )
        assert callable(beignet.statistics.jonckheere_terpstra_test_sample_size)

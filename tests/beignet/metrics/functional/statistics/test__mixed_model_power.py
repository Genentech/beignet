...
def test_mixed_model_power(batch_size, dtype):
    """Test mixed_model_power functional wrapper."""
    ...
    try:
        result_functional = beignet.metrics.functional.statistics.mixed_model_power(
            test_input,
        )
        result_direct = beignet.statistics.mixed_model_power(test_input)
        ...
        assert hasattr(beignet.metrics.functional.statistics, "mixed_model_power")
        assert hasattr(beignet.statistics, "mixed_model_power")
        assert callable(beignet.metrics.functional.statistics.mixed_model_power)
        assert callable(beignet.statistics.mixed_model_power)

...
def test_paired_t_test_sample_size(batch_size, dtype):
    """Test paired t-test sample size calculation."""
    ...
    result = beignet.statistics.paired_t_test_sample_size(effect_sizes, powers)
    ...
    result_out = beignet.statistics.paired_t_test_sample_size(
        effect_sizes,
        powers,
        out=out,
    )
    ...
    small_effect = beignet.statistics.paired_t_test_sample_size(
        torch.tensor(0.2, dtype=dtype),
        torch.tensor(0.8, dtype=dtype),
    )
    large_effect = beignet.statistics.paired_t_test_sample_size(
        torch.tensor(0.8, dtype=dtype),
        torch.tensor(0.8, dtype=dtype),
    )
    ...
    low_power = beignet.statistics.paired_t_test_sample_size(
        torch.tensor(0.5, dtype=dtype),
        torch.tensor(0.7, dtype=dtype),
    )
    high_power = beignet.statistics.paired_t_test_sample_size(
        torch.tensor(0.5, dtype=dtype),
    ...
    for alt in ["two-sided", "greater", "less"]:
        result_alt = beignet.statistics.paired_t_test_sample_size(
            torch.tensor(0.5, dtype=dtype),
            torch.tensor(0.8, dtype=dtype),
            alternative=alt,
        )
        assert result_alt >= 1.0

    # Test different alpha values
    alpha_05 = beignet.statistics.paired_t_test_sample_size(
        torch.tensor(0.5, dtype=dtype),
        torch.tensor(0.8, dtype=dtype),
        alpha=0.05,
    )
    alpha_01 = beignet.statistics.paired_t_test_sample_size(
        torch.tensor(0.5, dtype=dtype),
        torch.tensor(0.8, dtype=dtype),
        alpha=0.01,
    )
    ...
    result_grad = beignet.statistics.paired_t_test_sample_size(effect_grad, power_grad)
    ...
    compiled_func = torch.compile(
        beignet.statistics.paired_t_test_sample_size,
        fullgraph=True,
    )
    result_compiled = compiled_func(effect_sizes[:1], powers[:1])
    result_normal = beignet.statistics.paired_t_test_sample_size(
        effect_sizes[:1],
        powers[:1],
    )

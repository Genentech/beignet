...
from ..functional.statistics import wilcoxon_signed_rank_test_power


class WilcoxonSignedRankTestPower(Metric):
    r"""
    Compute statistical power for Wilcoxon signed-rank test.

    This metric calculates the statistical power for Wilcoxon signed-rank test,
    which is a non-parametric test for paired samples. It follows TorchMetrics
    ...
    --------
    >>> import torch
    >>> from beignet.metrics import WilcoxonSignedRankTestPower
    >>> metric = WilcoxonSignedRankTestPower()
    ...
    def compute(self) -> Tensor:
        ...
        return wilcoxon_signed_rank_test_power(
            effect_size,
            sample_size,
            alpha=self.alpha,
    ...
    def plot(
        ...
        **kwargs: Any,
        ...
        """Plot Wilcoxon signed-rank test power visualization.
        ...
        **kwargs : dict
        ...
        if plot_type == "power_curve":
            ...
            for es in effect_sizes:
                power = wilcoxon_signed_rank_test_power(
                    torch.tensor(es),
            ...
            if title is None:
                title = f"Wilcoxon Signed-Rank Test Power Analysis (α={self.alpha}, N={current_n})"
        else:

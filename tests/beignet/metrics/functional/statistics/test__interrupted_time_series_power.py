...
def test_interrupted_time_series_power(batch_size, dtype):
    """Test interrupted_time_series_power functional wrapper."""
    ...
    try:
        result_functional = (
            beignet.metrics.functional.statistics.interrupted_time_series_power(
                test_input,
            )
        )
        result_direct = beignet.statistics.interrupted_time_series_power(test_input)
        ...
        assert hasattr(beignet.statistics, "interrupted_time_series_power")
        assert callable(
            beignet.metrics.functional.statistics.interrupted_time_series_power,
        )
        assert callable(beignet.statistics.interrupted_time_series_power)

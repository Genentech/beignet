...
def test_proportional_hazards_model_power(batch_size, dtype):
    """Test proportional_hazards_model_power functional wrapper."""
    ...
    try:
        result_functional = (
            beignet.metrics.functional.statistics.proportional_hazards_model_power(
                test_input,
            )
        )
        result_direct = beignet.statistics.proportional_hazards_model_power(test_input)
        ...
        assert hasattr(beignet.statistics, "proportional_hazards_model_power")
        assert callable(
            beignet.metrics.functional.statistics.proportional_hazards_model_power,
        )
        assert callable(beignet.statistics.proportional_hazards_model_power)

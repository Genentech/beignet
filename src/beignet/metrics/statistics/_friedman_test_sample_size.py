...
from ..functional.statistics import friedman_test_sample_size


class FriedmanTestSampleSize(Metric):
    r"""
    Compute required sample size for Friedman test.

    This metric calculates the required sample size for Friedman test to achieve
    ...
    --------
    >>> import torch
    >>> from beignet.metrics import FriedmanTestSampleSize
    >>> metric = FriedmanTestSampleSize()
    ...
    def compute(self) -> Tensor:
        ...
        return friedman_test_sample_size(
            effect_size,
            groups,
            power=self.power,
            alpha=self.alpha,
    ...
    def plot(
        ...
        **kwargs: Any,
        ...
        """Plot Friedman test sample size visualization."""
        try:
        ...
        if plot_type == "sample_size_curve":
            ...
            ax.axhline(y=40, color="gray", linestyle=":", alpha=0.5, label="N = 40")
            ...
            if title is None:
                title = f"Friedman Test Sample Size (Power={self.power}, α={self.alpha}, Groups={current_groups})"
        else:

...
def test_polyder():
    ...
    for i in range(5):
        ...
        torch.testing.assert_close(
            beignet.polynomial._polytrim.polytrim(
                beignet.polynomial._polyder.polyder(tgt, order=0), tolerance=1e-6
            ),
    ...
    for i in range(5):
        for j in range(2, 5):
            ...
            res = beignet.polynomial._polyder.polyder(
                beignet.polynomial._polyint.polyint(tgt, m=j), order=j
            )
    ...
    for i in range(5):
        for j in range(2, 5):
            tgt = [0] * i + [1]
            res = beignet.polynomial._polyder.polyder(
                beignet.polynomial._polyint.polyint(tgt, m=j, scl=2), order=j, scale=0.5
            )
            numpy.testing.assert_almost_equal(
                beignet.polynomial._polytrim.polytrim(res, tolerance=1e-6),
                beignet.polynomial._polytrim.polytrim(tgt, tolerance=1e-6),
    ...
    numpy.testing.assert_almost_equal(
        beignet.polynomial._polyder.polyder(c2d, axis=0), tgt
    )

    tgt = numpy.vstack([beignet.polynomial._polyder.polyder(c) for c in c2d])
    numpy.testing.assert_almost_equal(
        beignet.polynomial._polyder.polyder(c2d, axis=1), tgt
    )

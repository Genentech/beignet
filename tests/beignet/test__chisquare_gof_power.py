...
def test_chisquare_gof_power(batch_size: int, dtype: torch.dtype) -> None:
    ...
    power = chisquare_gof_power(effect_size, sample_size, df, alpha=0.05)
    ...
    power_01 = chisquare_gof_power(effect_size, sample_size, df, alpha=0.01)
    power_05 = chisquare_gof_power(effect_size, sample_size, df, alpha=0.05)
    ...
    power_zero = chisquare_gof_power(zero_effect, sample_size, df, alpha=0.05)
    ...
    power_large = chisquare_gof_power(large_effect, large_sample, df, alpha=0.05)
    ...
    power_small = chisquare_gof_power(small_effect, sample_size, df, alpha=0.05)
    power_large = chisquare_gof_power(large_effect, sample_size, df, alpha=0.05)
    ...
    power_small = chisquare_gof_power(effect_size, small_sample, df, alpha=0.05)
    power_large = chisquare_gof_power(effect_size, large_sample, df, alpha=0.05)
    assert torch.all(power_large >= power_small)
    ...
    compiled_func = torch.compile(chisquare_gof_power, fullgraph=True)
    power_compiled = compiled_func(
        effect_size.detach(), sample_size.detach(), df.detach(), alpha=0.05
    )
    power_regular = chisquare_gof_power(
        effect_size.detach(), sample_size.detach(), df.detach(), alpha=0.05
    )
...
def test_chisquare_gof_power_statsmodels_comparison() -> None:
    ...
    # Our implementation
    power_beignet = chisquare_gof_power(

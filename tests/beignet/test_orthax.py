...
def test_cheb2poly():
    for index in range(10):
        numpy.testing.assert_array_almost_equal(
            beignet.orthax.cheb2poly([0] * index + [1]), chebcoefficients[index]
        )
        ...
        for j in range(5):
            ...
            numpy.testing.assert_array_equal(
                beignet.orthax.chebtrim(
                    beignet.orthax.chebadd([0] * i + [1], [0] * j + [1]), tol=1e-6
                ),
...
def test_chebcompanion():
    ...
    for index in range(1, 5):
        numpy.testing.assert_(
            beignet.orthax.chebcompanion([0] * index + [1]).shape == (index, index)
        )
    ...
    for j in range(5):
        for k in range(5):
            quo, rem = beignet.orthax.chebdiv(
                beignet.orthax.chebadd([0] * j + [1], [0] * k + [1]), [0] * j + [1]
            )
            numpy.testing.assert_array_equal(
                beignet.orthax.chebtrim(
                    beignet.orthax.chebadd(
                        beignet.orthax.chebmul(quo, [0] * j + [1]), rem
                    ),
                    tol=1e-6,
                ),
                beignet.orthax.chebtrim(
                    beignet.orthax.chebadd([0] * j + [1], [0] * k + [1]), tol=1e-6
                ),
            )
...
def test_hermval():
    ...
    for i in range(10):
        ...
        numpy.testing.assert_array_almost_equal(res, tgt, decimal=3)
...
def test_legfit():
    ...
    numpy.testing.assert_array_almost_equal(
        beignet.orthax.legval(x, beignet.orthax.legfit(x, y, 4)), y
    )

    numpy.testing.assert_array_almost_equal(
        beignet.orthax.legval(x, beignet.orthax.legfit(x, y, (0, 2, 4))), y
    )
...
def test_legval3d():
    ...
    numpy.testing.assert_array_almost_equal(
        beignet.orthax.legval3d(x1, x2, x3, c3d), y1 * y2 * y3
    )

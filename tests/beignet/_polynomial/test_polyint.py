...
def test_polyint():
    ...
    numpy.testing.assert_raises(
        TypeError, beignet.polynomial._polyint.polyint, [1, 1], 1.0
    )
    ...
    numpy.testing.assert_almost_equal(
        beignet.polynomial._polyint.polyint(c2d, axis=0),
        numpy.vstack([beignet.polynomial._polyint.polyint(c) for c in c2d.T]).T,
    )

    numpy.testing.assert_almost_equal(
        beignet.polynomial._polyint.polyint(c2d, axis=1),
        numpy.vstack([beignet.polynomial._polyint.polyint(c) for c in c2d]),
    )

    numpy.testing.assert_almost_equal(
        beignet.polynomial._polyint.polyint(c2d, k=3, axis=1),
        numpy.vstack([beignet.polynomial._polyint.polyint(c, k=3) for c in c2d]),
    )

...
def test_kolmogorov_smirnov_test_power(batch_size, dtype):
    """Test kolmogorov_smirnov_test_power functional wrapper."""
    ...
    try:
        result_functional = (
            beignet.metrics.functional.statistics.kolmogorov_smirnov_test_power(
                test_input,
            )
        )
        result_direct = beignet.statistics.kolmogorov_smirnov_test_power(test_input)
        ...
        assert hasattr(beignet.statistics, "kolmogorov_smirnov_test_power")
        assert callable(
            beignet.metrics.functional.statistics.kolmogorov_smirnov_test_power,
        )
        assert callable(beignet.statistics.kolmogorov_smirnov_test_power)

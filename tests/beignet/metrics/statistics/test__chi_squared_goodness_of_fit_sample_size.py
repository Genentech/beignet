...
def test_chi_squared_goodness_of_fit_sample_size(
    ...
    """Test ChiSquaredGoodnessOfFitSampleSize TorchMetrics class."""
    # Initialize the metric
    metric = beignet.metrics.statistics.ChiSquaredGoodnessOfFitSampleSize(alpha=alpha)
    ...
    assert torch.all(output >= 1.0)  # Should be at least 1
    ...
    if dtype == torch.float32:
        ...
        metric_64 = beignet.metrics.statistics.ChiSquaredGoodnessOfFitSampleSize(
            alpha=alpha,
        )
    ...
    metric_small = beignet.metrics.statistics.ChiSquaredGoodnessOfFitSampleSize(
        alpha=alpha,
    )
    ...
    metric_large = beignet.metrics.statistics.ChiSquaredGoodnessOfFitSampleSize(
        alpha=alpha,
    )
    ...
    assert "ChiSquaredGoodnessOfFitSampleSize" in repr_str
    assert f"alpha={alpha}" in repr_str

    # Test device consistency
    if torch.cuda.is_available():
        device = torch.device("cuda")
        metric_cuda = beignet.metrics.statistics.ChiSquaredGoodnessOfFitSampleSize(
            alpha=alpha,
        ).to(device)
    ...
    metric_grad = beignet.metrics.statistics.ChiSquaredGoodnessOfFitSampleSize(
        alpha=alpha,
    )

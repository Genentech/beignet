...
import beignet._add_legendre_series
import beignet._differentiate_legendre_series
import beignet._divide_legendre_series
import beignet._evaluate_2d_legendre_series
import beignet._evaluate_3d_legendre_series
import beignet._evaluate_legendre_series
import beignet._evaluate_power_series
import beignet._fit_legendre_series
import beignet._gauss_legendre_quadrature
import beignet._integrate_legendre_series
import beignet._legendre_series_from_roots
import beignet._legendre_series_roots
import beignet._legendre_series_to_power_series
import beignet._legendre_series_vandermonde
import beignet._multiply_legendre_series
import beignet._power_series_to_legendre_series
import beignet._subtract_legendre_series
import beignet._trim_legendre_series
...
def trim(x):
    return beignet.polynomial._legtrim.trim_legendre_series(x, tol=1e-6)
...
class TestArithmetic:
    ...
    def test_legsub(self):
        for i in range(5):
            for j in range(5):
                ...
                tgt[i] += 1
                tgt[j] -= 1
                res = beignet.polynomial._legsub.subtract_legendre_series(
                    [0] * i + [1], [0] * j + [1]
                )
                numpy.testing.assert_equal(trim(res), trim(tgt), err_msg=msg)
    ...
    def test_legmul(self):
        ...
        for i in range(5):
            pol1 = [0] * i + [1]
            val1 = beignet.polynomial._legval.evaluate_legendre_series(self.x, pol1)
            for j in range(5):
                msg = f"At i={i}, j={j}"
                pol2 = [0] * j + [1]
                val2 = beignet.polynomial._legval.evaluate_legendre_series(self.x, pol2)
                pol3 = beignet.polynomial._legmul.multiply_legendre_series(pol1, pol2)
                val3 = beignet.polynomial._legval.evaluate_legendre_series(self.x, pol3)
    ...
    def test_legdiv(self):
        for i in range(5):
            for j in range(5):
                ...
                quo, rem = beignet.polynomial._legdiv.divide_legendre_series(tgt, ci)
                res = beignet.polynomial._legadd.add_legendre_series(
                    beignet.polynomial._legmul.multiply_legendre_series(quo, ci), rem
                )
                numpy.testing.assert_equal(trim(res), trim(tgt), err_msg=msg)
...
class TestEvaluation:
    ...
    def test_legval(self):
        ...
        numpy.testing.assert_equal(
            beignet.polynomial._legval.evaluate_legendre_series([], [1]).size, 0
        )

        # check normal input)
        x = numpy.linspace(-1, 1)
        y = [beignet.polynomial._polyval.evaluate_power_series(x, c) for c in Llist]
        for i in range(10):
            msg = f"At i={i}"
            tgt = y[i]
            res = beignet.polynomial._legval.evaluate_legendre_series(x, [0] * i + [1])
        ...
        for i in range(3):
            ...
            x = numpy.zeros(dims)
            numpy.testing.assert_equal(
                beignet.polynomial._legval.evaluate_legendre_series(x, [1]).shape, dims
            )
            numpy.testing.assert_equal(
                beignet.polynomial._legval.evaluate_legendre_series(x, [1, 0]).shape,
                dims,
            )
            numpy.testing.assert_equal(
                beignet.polynomial._legval.evaluate_legendre_series(x, [1, 0, 0]).shape,
                dims,
            )
    ...
    def test_legval2d(self):
        ...
        # test values
        tgt = y1 * y2
        res = beignet.polynomial._legval2d.evaluate_2d_legendre_series(x1, x2, self.c2d)
        numpy.testing.assert_almost_equal(res, tgt)

        # test shape
        z = numpy.ones((2, 3))
        res = beignet.polynomial._legval2d.evaluate_2d_legendre_series(z, z, self.c2d)
    ...
    def test_legval3d(self):
        ...
        # test values
        tgt = y1 * y2 * y3
        res = beignet.polynomial._legval3d.evaluate_3d_legendre_series(
            x1, x2, x3, self.c3d
        )
        numpy.testing.assert_almost_equal(res, tgt)
...
class TestIntegral:
    def test_legint(self):
        ...
        for i in range(2, 5):
            k = [0] * (i - 2) + [1]
            res = beignet.polynomial._legint.integrate_legendre_series([0], m=i, k=k)
        ...
        for i in range(5):
            ...
            res = beignet.polynomial._leg2poly.leg_to_power_series(legint)
        ...
        for i in range(5):
            ...
            numpy.testing.assert_almost_equal(
                beignet.polynomial._legval.evaluate_legendre_series(-1, legint), i
            )
    ...
    def test_legint_axis(self):
        ...
        res = beignet.polynomial._legint.integrate_legendre_series(c2d, axis=0)
        numpy.testing.assert_almost_equal(res, tgt)

        tgt = numpy.vstack(
            [beignet.polynomial._legint.integrate_legendre_series(c) for c in c2d]
        )
        res = beignet.polynomial._legint.integrate_legendre_series(c2d, axis=1)
        numpy.testing.assert_almost_equal(res, tgt)

        tgt = numpy.vstack(
            [beignet.polynomial._legint.integrate_legendre_series(c, k=3) for c in c2d]
        )
        res = beignet.polynomial._legint.integrate_legendre_series(c2d, k=3, axis=1)
        numpy.testing.assert_almost_equal(res, tgt)

    def test_legint_zerointord(self):
        numpy.testing.assert_equal(
            beignet.polynomial._legint.integrate_legendre_series((1, 2, 3), 0),
            (1, 2, 3),
        )
...
class TestDerivative:
    def test_legder(self):
        ...
        numpy.testing.assert_raises(
            TypeError,
            beignet.polynomial._legder.differentiate_legendre_series,
            [0],
            0.5,
        )
        numpy.testing.assert_raises(
...
class TestVander:
    ...
    def test_legvander(self):
        # check for 1d x
        x = numpy.arange(3)
        v = beignet.polynomial._legvander.legendre_series_vandermonde(x, 3)
        numpy.testing.assert_(v.shape == (3, 4))
        for i in range(4):
            coef = [0] * i + [1]
            numpy.testing.assert_almost_equal(
                v[..., i], beignet.polynomial._legval.evaluate_legendre_series(x, coef)
            )
    ...
    def test_legvander2d(self):
        ...
        tgt = beignet.polynomial._legval2d.evaluate_2d_legendre_series(x1, x2, c)
    ...
    def test_legvander3d(self):
        ...
        tgt = beignet.polynomial._legval3d.evaluate_3d_legendre_series(x1, x2, x3, c)
        res = numpy.dot(van, c.flat)
        numpy.testing.assert_almost_equal(res, tgt)
    ...
    def test_legvander_negdeg(self):
        numpy.testing.assert_raises(
            ValueError,
            beignet.polynomial._legvander.legendre_series_vandermonde,
            (1, 2, 3),
            -1,
        )
...
class TestFitting:
    def test_legfit(self):
        ...
        # Test exceptions
        numpy.testing.assert_raises(
            ValueError, beignet.polynomial._legfit.fit_legendre_series, [1], [1], -1
        )
        numpy.testing.assert_raises(
            TypeError, beignet.polynomial._legfit.fit_legendre_series, [[1]], [1], 0
        )
        numpy.testing.assert_raises(
            TypeError, beignet.polynomial._legfit.fit_legendre_series, [], [1], 0
        )
        numpy.testing.assert_raises(
            TypeError, beignet.polynomial._legfit.fit_legendre_series, [1], [[[1]]], 0
        )
        numpy.testing.assert_raises(
            TypeError, beignet.polynomial._legfit.fit_legendre_series, [1, 2], [1], 0
        )
        numpy.testing.assert_raises(
            TypeError, beignet.polynomial._legfit.fit_legendre_series, [1], [1, 2], 0
        )
        numpy.testing.assert_raises(
            TypeError,
            beignet.polynomial._legfit.fit_legendre_series,
            [1],
            [1],
            0,
            w=[[1]],
        ...
        numpy.testing.assert_raises(
            TypeError, beignet.polynomial._legfit.fit_legendre_series, [1], [1], []
        )
        ...
        numpy.testing.assert_almost_equal(
            beignet.polynomial._legval.evaluate_legendre_series(x, coef3), y
        )
        ...
        numpy.testing.assert_almost_equal(
            beignet.polynomial._legval.evaluate_legendre_series(x, coef4), y
        )
        ...
        #
        coef2d = beignet.polynomial._legfit.fit_legendre_series(
        ...
        w[1::2] = 1
        y[0::2] = 0
        wcoef3 = beignet.polynomial._legfit.fit_legendre_series(x, yw, 3, w=w)
        numpy.testing.assert_almost_equal(wcoef3, coef3)
        wcoef3 = beignet.polynomial._legfit.fit_legendre_series(
            x, yw, [0, 1, 2, 3], w=w
        )
        numpy.testing.assert_almost_equal(wcoef3, coef3)
        #
        wcoef2d = beignet.polynomial._legfit.fit_legendre_series(
        ...
        x = [1, 1j, -1, -1j]
        numpy.testing.assert_almost_equal(
            beignet.polynomial._legfit.fit_legendre_series(x, x, 1), [0, 1]
        )
        numpy.testing.assert_almost_equal(
            beignet.polynomial._legfit.fit_legendre_series(x, x, [0, 1]), [0, 1]
        )
        ...
        numpy.testing.assert_almost_equal(
            beignet.polynomial._legval.evaluate_legendre_series(x, coef1), y
        )
        coef2 = beignet.polynomial._legfit.fit_legendre_series(x, y, [0, 2, 4])
        numpy.testing.assert_almost_equal(
            beignet.polynomial._legval.evaluate_legendre_series(x, coef2), y
        )
...
class TestGauss:
    def test_100(self):
        ...
        v = beignet.polynomial._legvander.legendre_series_vandermonde(x, 99)
...
class TestMisc:
    def test_legfromroots(self):
        res = beignet.polynomial._legfromroots.legendre_series_from_roots([])
        numpy.testing.assert_almost_equal(trim(res), [1])
        for i in range(1, 5):
            roots = numpy.cos(numpy.linspace(-numpy.pi, 0, 2 * i + 1)[1::2])
            pol = beignet.polynomial._legfromroots.legendre_series_from_roots(roots)
            res = beignet.polynomial._legval.evaluate_legendre_series(roots, pol)
            tgt = 0
            numpy.testing.assert_(len(pol) == i + 1)
            numpy.testing.assert_almost_equal(
                beignet.polynomial._leg2poly.leg_to_power_series(pol)[-1], 1
            )
            numpy.testing.assert_almost_equal(res, tgt)
    ...
    def test_legroots(self):
        ...
        for i in range(2, 5):
            ...
            res = beignet.polynomial._legroots.legendre_series_roots(
                beignet.polynomial._legfromroots.legendre_series_from_roots(tgt)
            )
            numpy.testing.assert_almost_equal(trim(res), trim(tgt))
    ...
    def test_legtrim(self):
        ...
        # Test exceptions
        numpy.testing.assert_raises(
            ValueError, beignet.polynomial._legtrim.trim_legendre_series, coef, -1
        )

        # Test results
        numpy.testing.assert_equal(
            beignet.polynomial._legtrim.trim_legendre_series(coef), coef[:-1]
        )
        numpy.testing.assert_equal(
            beignet.polynomial._legtrim.trim_legendre_series(coef, 1), coef[:-3]
        )
        numpy.testing.assert_equal(
            beignet.polynomial._legtrim.trim_legendre_series(coef, 2), [0]
        )
    ...
    def test_leg2poly(self):
        for i in range(10):
            numpy.testing.assert_almost_equal(
                beignet.polynomial._leg2poly.leg_to_power_series([0] * i + [1]),
                Llist[i],
            )
    ...
    def test_poly2leg(self):
        for i in range(10):
            numpy.testing.assert_almost_equal(
                beignet.polynomial._poly2leg.power_series_to_legendre_series(Llist[i]),
                [0] * i + [1],
            )

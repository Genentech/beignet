...
def test_proportion_two_sample_sample_size(
    ...
    """Test proportion_two_sample_sample_size functional wrapper."""
    effect_size_tensor = torch.full((batch_size,), effect_size, dtype=dtype)
    power_tensor = torch.full((batch_size,), power, dtype=dtype)

    result_functional = (
        beignet.metrics.functional.statistics.proportion_two_sample_sample_size(
            effect_size_tensor,
            power_tensor,
            alpha=alpha,
        )
    )
    result_direct = beignet.statistics.proportion_two_sample_sample_size(
        effect_size_tensor,
        power_tensor,
        alpha=alpha,
    )
    ...
    assert torch.all(result_functional >= 2.0)

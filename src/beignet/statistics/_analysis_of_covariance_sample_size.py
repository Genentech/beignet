...
def analysis_of_covariance_sample_size(
    ...
    *,
    ...
    dtype = (
        torch.float64
        if any(t.dtype == torch.float64 for t in (f, k, R2, p))
        else torch.float32
    )
    f = torch.clamp(f.to(dtype), min=1e-8)
    ...
    N0 = lam0 * torch.clamp(1.0 - R2, min=torch.finfo(dtype).eps) / (f**2)
    N0 = torch.clamp(N0, min=k + p + 2.0)

    N_curr = N0
    for _ in range(8):
        df2 = torch.clamp(N_curr - k - p, min=1.0)
        lambda_nc = N_curr * f**2 / torch.clamp(1.0 - R2, min=torch.finfo(dtype).eps)

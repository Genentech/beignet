...
@cache
def _make_ambiguous_atom_swap_indices() -> list[list[int]]:
    out = []
...
@cache
def _make_atom_thin_is_ambiguous_mask() -> list[int]:
    out = []
    for res in STANDARD_RESIDUES:
        atoms = ATOM_THIN_ATOMS[AMINO_ACID_1_TO_3[res]]
        res_swaps = _AMBIGUOUS_ATOM_SWAPS.get(AMINO_ACID_1_TO_3[res], {})
...
def rename_symmetric_atoms(
    ...
    atom_thin_xyz_alt, _ = swap_ambiguous_atom_thin_atoms(
    ...
    atom_thin_is_ambiguous = torch.as_tensor(_make_atom_thin_is_ambiguous_mask())[
        residue_type
    ]
    pair_mask = (
        atom_thin_is_ambiguous[..., None, :, None] & atom_thin_mask[..., None, :, None]
    ) & (
        ~atom_thin_is_ambiguous[..., None, :, None, :]

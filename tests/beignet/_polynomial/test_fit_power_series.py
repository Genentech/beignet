...
def test_fit_power_series():
    ...
    x = [1, 1j, -1, -1j]
    torch.testing.assert_close(beignet.polynomial.fit_power_series(x, x, 1), [0, 1])
    torch.testing.assert_close(
    ...
    torch.testing.assert_close(beignet.polynomial.evaluate_power_series_1d(x, coef1), y)
    coef2 = beignet.polynomial.fit_power_series(x, y, [0, 2, 4])
    torch.testing.assert_close(beignet.polynomial.evaluate_power_series_1d(x, coef2), y)

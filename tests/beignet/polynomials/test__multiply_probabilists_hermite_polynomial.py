...
def test_multiply_probabilists_hermite_polynomial(float64):
    for index in range(5):
        input = torch.linspace(-3, 3, 100)

        val1 = beignet.polynomials.evaluate_probabilists_hermite_polynomial(
            input,
            torch.tensor([0.0] * index + [1.0]),
        )

        for k in range(5):
            val2 = beignet.polynomials.evaluate_probabilists_hermite_polynomial(
                input,

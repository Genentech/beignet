...
def test_analysis_of_variance_power(
    ...
    """Test AnalysisOfVariancePower TorchMetrics class."""
    # Initialize the metric
    metric = beignet.metrics.statistics.AnalysisOfVariancePower(alpha=alpha)
    ...
    metric.update(effect_size_tensor * 0.5, sample_size_tensor, groups_tensor)
    ...
    if dtype == torch.float32:
        ...
        metric_64 = beignet.metrics.statistics.AnalysisOfVariancePower(alpha=alpha)
        metric_64.update(float64_effect, sample_size_tensor, groups_tensor)
    ...
    metric_small = beignet.metrics.statistics.AnalysisOfVariancePower(alpha=alpha)
    metric_small.update(small_effect, sample_size_tensor, groups_tensor)
    ...
    metric_large = beignet.metrics.statistics.AnalysisOfVariancePower(alpha=alpha)
    metric_large.update(large_effect, sample_size_tensor, groups_tensor)
    power_large = metric_large.compute()
    assert torch.all(power_large > 0.5)  # Should be higher power

    # Test repr
    repr_str = repr(metric)
    assert "AnalysisOfVariancePower" in repr_str
    assert f"alpha={alpha}" in repr_str

    # Test device consistency
    if torch.cuda.is_available():
        device = torch.device("cuda")
        metric_cuda = beignet.metrics.statistics.AnalysisOfVariancePower(
            alpha=alpha,
        ).to(device)
        ...
        metric_cuda.update(effect_cuda, sample_cuda, groups_cuda)
    ...
    metric_grad = beignet.metrics.statistics.AnalysisOfVariancePower(alpha=alpha)
    metric_grad.update(effect_grad, sample_grad, groups_grad)

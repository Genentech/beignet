...
def test_intraclass_correlation_power(batch_size, dtype):
    """Test intraclass_correlation_power functional wrapper."""
    ...
    try:
        result_functional = (
            beignet.metrics.functional.statistics.intraclass_correlation_power(
                test_input,
            )
        )
        result_direct = beignet.statistics.intraclass_correlation_power(test_input)
        ...
        assert hasattr(beignet.statistics, "intraclass_correlation_power")
        assert callable(
            beignet.metrics.functional.statistics.intraclass_correlation_power,
        )
        assert callable(beignet.statistics.intraclass_correlation_power)

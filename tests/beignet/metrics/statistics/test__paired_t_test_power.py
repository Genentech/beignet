...
from beignet.metrics.statistics import PairedTTestPower
...
def test_paired_t_test_power_metric(batch_size, dtype):
    """Test PairedTTestPower metric wrapper."""
    # Create reasonable test parameters
    effect_size = (
        torch.abs(torch.randn(batch_size, dtype=dtype)) * 0.8 + 0.2
    )  # Between 0.2 and 1.0
    sample_size = torch.randint(10, 50, (batch_size,), dtype=dtype)

    alpha = 0.05
    alternative = "two-sided"

    # Initialize metric
    metric = PairedTTestPower(alpha=alpha, alternative=alternative)
    ...
    try:
        PairedTTestPower(alpha=1.5)
        assert False, "Should raise ValueError for invalid alpha"
    except ValueError:
        pass  # Expected

    try:
        PairedTTestPower(alternative="invalid")

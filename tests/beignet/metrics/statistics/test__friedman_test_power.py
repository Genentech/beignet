...
from beignet.metrics.statistics import FriedmanTestPower


@given(
    effect_size=st.floats(min_value=0.1, max_value=1.0),
    sample_size=st.integers(min_value=15, max_value=200),
    groups=st.integers(min_value=3, max_value=6),
    alpha=st.floats(min_value=0.01, max_value=0.1),
    dtype=st.sampled_from([torch.float32, torch.float64])
)
def test_friedman_test_power(
    effect_size, sample_size, groups, alpha, dtype
):
    metric = FriedmanTestPower(alpha=alpha)

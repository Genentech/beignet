...
from beignet.metrics.statistics import CliffsD
...
def test_cliffs_delta_metric(batch_size, dtype):
    """Test CliffsD metric wrapper."""
    # Create test data with some effect
    group1 = torch.randn(batch_size, 20, dtype=dtype)
    group2 = torch.randn(batch_size, 20, dtype=dtype) + 0.5
    ...
    # Compare with functional implementation - concatenate the same way the metric does
    group1_cat = torch.cat([group1], dim=-1)
    group2_cat = torch.cat([group2], dim=-1)
    result_functional = beignet.metrics.functional.statistics.cliffs_delta(
        group1_cat,
        group2_cat,
    )

...
def test_intraclass_correlation_sample_size(batch_size, dtype):
    """Test intraclass_correlation_sample_size functional wrapper."""
    ...
    try:
        result_functional = (
            beignet.metrics.functional.statistics.intraclass_correlation_sample_size(
                test_input,
            )
        )
        result_direct = beignet.statistics.intraclass_correlation_sample_size(
            test_input,
        )
        ...
        assert hasattr(beignet.statistics, "intraclass_correlation_sample_size")
        assert callable(
            beignet.metrics.functional.statistics.intraclass_correlation_sample_size,
        )
        assert callable(beignet.statistics.intraclass_correlation_sample_size)

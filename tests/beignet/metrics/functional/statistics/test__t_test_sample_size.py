...
def test_t_test_sample_size(batch_size, effect_size, power, alpha, dtype):
    """Test t_test_sample_size functional wrapper."""
    effect_size_tensor = torch.full((batch_size,), effect_size, dtype=dtype)

    result_functional = beignet.metrics.functional.statistics.t_test_sample_size(
        effect_size_tensor,
        power=power,
        alpha=alpha,
    )
    result_direct = beignet.statistics.t_test_sample_size(
        effect_size_tensor,
        power=power,
        alpha=alpha,
    )

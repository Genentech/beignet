...
class IntegratePhysicistsHermitePolynomialBenchmark:
    params = [
        [10, 100, 1000, 10000],
    ...
    def time_integrate_physicists_hermite_polynomial(self, batch_size, dtype):
        self.func(
            self.input, self.order, self.k, self.lower_bound, self.scale, self.axis
        )

    def peak_memory_integrate_physicists_hermite_polynomial(self, batch_size, dtype):
        self.func(
            self.input, self.order, self.k, self.lower_bound, self.scale, self.axis
        )

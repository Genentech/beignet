...
def test_independent_z_test_sample_size(batch_size, effect_size, power, alpha, dtype):
    """Test independent_z_test_sample_size functional wrapper."""
    effect_size_tensor = torch.full((batch_size,), effect_size, dtype=dtype)
    power_tensor = torch.full((batch_size,), power, dtype=dtype)

    result_functional = (
        beignet.metrics.functional.statistics.independent_z_test_sample_size(
            effect_size_tensor,
            power_tensor,
            alpha=alpha,
        )
    )
    result_direct = beignet.statistics.independent_z_test_sample_size(
        effect_size_tensor,
        power_tensor,
        alpha=alpha,
    )

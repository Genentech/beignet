...
def test_wilcoxon_signed_rank_test_sample_size(batch_size, dtype):
    """Test wilcoxon_signed_rank_test_sample_size functional wrapper."""
    ...
    try:
        result_functional = (
            beignet.metrics.functional.statistics.wilcoxon_signed_rank_test_sample_size(
                test_input,
            )
        )
        result_direct = beignet.statistics.wilcoxon_signed_rank_test_sample_size(
            test_input,
        )
        ...
        assert hasattr(beignet.statistics, "wilcoxon_signed_rank_test_sample_size")
        assert callable(
            beignet.metrics.functional.statistics.wilcoxon_signed_rank_test_sample_size,
        )
        assert callable(beignet.statistics.wilcoxon_signed_rank_test_sample_size)

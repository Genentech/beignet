...
class Atom3DPPIToSequenceAndContactMap(Transform):
    """
    Transforms an Atom3DPPIDataset output (feature, target) to its constituent
    protein sequences and a 2D contact map (sequenceA, sequenceB, contact_map).
    Use as a joint_transform arg when instantiating a Dataset
    """
    ...
    def transform(
        ...
        """
        Return list of tuples of (id, sequence) for different chains of
        monomers in a given dataframe Adapted from atom3d.protein.sequence
        """
        ...
        ixs = list(zip(target["residue0"], target["residue1"], strict=False))
...
class Atom3DPPIToSequence(Transform):
    ...
    def transform(
        ...
        """
        Return a tuple of sequences for different chains of monomers in a
        given dataframe Adapted from atom3d.protein.sequence
        """
        feat = inpt
        # Keep only alpha carbons (CA) of standard residues
        feat = feat[feat["name"] == "CA"].drop_duplicates()

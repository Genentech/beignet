    ...
    **kwargs
    ...
    def plot(
        ...
        **kwargs: Any,
        ...
        """Plot Cohen's f visualization showing group means.

        Parameters
        ----------
        plot_type : str, default="means"
        ...
        **kwargs : dict
        ...
        if plot_type == "means":
            ...
            if title is None:
                title = f"Group Means Comparison (Cohen's f = {float(effect_size):.3f})"
            ...
            # Color code by effect size magnitude (Cohen's conventions for f)
            color = "lightblue"
            if abs(effect_sizes[0]) >= 0.4:
                color = "red"  # Large effect
            elif abs(effect_sizes[0]) >= 0.25:

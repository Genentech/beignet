...
from beignet.datasets import QM7bDataset


class BenchQM7bDataset:
    params = [
        [10, 100, 1000],
    ]
    ...
    def setup(self, batch_size):
        self.dataset = QM7bDataset(
            root=os.getenv("BEIGNET_BENCHMARKS_DATASET_ROOT", "/tmp") + "/qm7b",
            download=True,
            transform=None,
    ...
    def time___init__(self, batch_size):
        QM7bDataset(
            root=os.getenv("BEIGNET_BENCHMARKS_DATASET_ROOT", ".") + "/qm7b",
            download=True,
            transform=None,
            target_transform=None,
        )

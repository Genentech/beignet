...
from beignet._chisquare_gof_power import chisquare_gof_power
from beignet._chisquare_gof_sample_size import chisquare_gof_sample_size
...
def test_chisquare_gof_sample_size(batch_size: int, dtype: torch.dtype) -> None:
    ...
    sample_size = chisquare_gof_sample_size(effect_size, df, power=power, alpha=alpha)
    ...
    sample_size_low = chisquare_gof_sample_size(effect_size, df, power=0.6, alpha=alpha)
    sample_size_high = chisquare_gof_sample_size(
    ...
    sample_size_strict = chisquare_gof_sample_size(
        effect_size, df, power=power, alpha=0.01
    )
    sample_size_lenient = chisquare_gof_sample_size(
    ...
    sample_size_small = chisquare_gof_sample_size(
        small_effect, df, power=power, alpha=alpha
    )
    sample_size_large = chisquare_gof_sample_size(
    ...
    computed_power = chisquare_gof_power(effect_size, sample_size, df, alpha=alpha)
    ...
    compiled_func = torch.compile(chisquare_gof_sample_size, fullgraph=True)
    sample_size_compiled = compiled_func(
        effect_size.detach(), df.detach(), power=power, alpha=alpha
    )
    sample_size_regular = chisquare_gof_sample_size(
        effect_size.detach(), df.detach(), power=power, alpha=alpha
    )
    ...
    result = chisquare_gof_sample_size(
    ...
    large_sample = chisquare_gof_sample_size(tiny_effect, df, power=power, alpha=alpha)
    ...
    small_sample = chisquare_gof_sample_size(big_effect, df, power=power, alpha=alpha)

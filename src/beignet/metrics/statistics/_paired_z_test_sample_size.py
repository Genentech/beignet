...
from ..functional.statistics import paired_z_test_sample_size


class PairedZTestSampleSize(Metric):
    r"""
    Compute required sample size for paired z-test.

    This metric calculates the required sample size for paired z-test to achieve
    ...
    --------
    >>> import torch
    >>> from beignet.metrics import PairedZTestSampleSize
    >>> metric = PairedZTestSampleSize()
    ...
    def compute(self) -> Tensor:
        ...
        return paired_z_test_sample_size(
            effect_size,
            power=self.power,
            alpha=self.alpha,
    ...
    def plot(
        ...
        **kwargs: Any,
        ...
        """Plot paired z-test sample size visualization."""
        try:
        ...
        if plot_type == "sample_size_curve":
            ...
            if title is None:
                title = (
                    f"Paired Z-Test Sample Size (Power={self.power}, α={self.alpha})"
                )
        else:

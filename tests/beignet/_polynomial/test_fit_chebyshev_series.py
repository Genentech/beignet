...
def test_fit_chebyshev_series():
    ...
    coef3 = beignet.polynomial.fit_chebyshev_series(
        numpy.linspace(0, 2), f(numpy.linspace(0, 2)), 3
    )
    torch.testing.assert_close(len(coef3), 4)
    torch.testing.assert_close(
        beignet.polynomial.evaluate_chebyshev_series_1d(numpy.linspace(0, 2), coef3),
    ...
    coef4 = beignet.polynomial.fit_chebyshev_series(
        numpy.linspace(0, 2), f(numpy.linspace(0, 2)), 4
    )
    torch.testing.assert_close(len(coef4), 5)
    torch.testing.assert_close(
        beignet.polynomial.evaluate_chebyshev_series_1d(numpy.linspace(0, 2), coef4),
        f(numpy.linspace(0, 2)),
    )
    coef4 = beignet.polynomial.fit_chebyshev_series(
    ...
    wcoef3 = beignet.polynomial.fit_chebyshev_series(numpy.linspace(0, 2), yw, 3, w=w)
    torch.testing.assert_close(wcoef3, coef3)
    wcoef3 = beignet.polynomial.fit_chebyshev_series(
    ...
    torch.testing.assert_close(
        beignet.polynomial.fit_chebyshev_series([1, 1j, -1, -1j], [1, 1j, -1, -1j], 1),
        [0, 1],
    )
    torch.testing.assert_close(
        beignet.polynomial.fit_chebyshev_series(

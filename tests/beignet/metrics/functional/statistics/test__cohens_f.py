...
def test_cohens_f(batch_size, sample_size_per_group, num_groups, dtype):
    """Test cohens_f functional wrapper."""
    groups = []
    for i in range(num_groups):
        group = torch.randn(batch_size, sample_size_per_group, dtype=dtype) + float(i)
        groups.append(group)

    result_functional = beignet.metrics.functional.statistics.cohens_f(groups)

    # Manually compute between-groups and within-groups standard deviations
    ...
    result_direct = beignet.statistics.cohens_f(between_groups_std, pooled_within_std)

...
def test_mixed_model_sample_size(batch_size, dtype):
    """Test mixed_model_sample_size functional wrapper."""
    ...
    try:
        result_functional = (
            beignet.metrics.functional.statistics.mixed_model_sample_size(test_input)
        )
        result_direct = beignet.statistics.mixed_model_sample_size(test_input)
        ...
        assert hasattr(beignet.metrics.functional.statistics, "mixed_model_sample_size")
        assert hasattr(beignet.statistics, "mixed_model_sample_size")
        assert callable(beignet.metrics.functional.statistics.mixed_model_sample_size)
        assert callable(beignet.statistics.mixed_model_sample_size)

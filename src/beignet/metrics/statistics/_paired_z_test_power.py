...
from ..functional.statistics import paired_z_test_power


class PairedZTestPower(Metric):
    r"""
    Compute statistical power for paired z-test.

    This metric calculates the statistical power for paired z-test, which compares
    paired observations when the population standard deviation is known.
    ...
    --------
    >>> import torch
    >>> from beignet.metrics import PairedZTestPower
    >>> metric = PairedZTestPower()
    ...
    def plot(
        ...
        **kwargs: Any,
        ...
        """Plot paired z-test power visualization."""
        try:
        ...
        if plot_type == "power_curve":
            ...
            if title is None:
                title = f"Paired Z-Test Power Analysis (α={self.alpha}, N={current_n})"
        else:

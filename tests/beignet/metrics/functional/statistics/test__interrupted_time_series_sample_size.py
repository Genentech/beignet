...
def test_interrupted_time_series_sample_size(batch_size, dtype):
    """Test interrupted_time_series_sample_size functional wrapper."""
    ...
    try:
        result_functional = (
            beignet.metrics.functional.statistics.interrupted_time_series_sample_size(
                test_input,
            )
        )
        result_direct = beignet.statistics.interrupted_time_series_sample_size(
            test_input,
        )
        ...
        assert hasattr(beignet.statistics, "interrupted_time_series_sample_size")
        assert callable(
            beignet.metrics.functional.statistics.interrupted_time_series_sample_size,
        )
        assert callable(beignet.statistics.interrupted_time_series_sample_size)

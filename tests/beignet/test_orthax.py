...
def test_chebint():
    ...
    for i in range(2, 5):
        ...
        numpy.testing.assert_array_almost_equal(
            beignet.orthax.chebtrim(
                beignet.orthax.chebint(array([0]), m=i, k=k), tol=0.000001
            ),
            [0, 1],
        )
            ...
            beignet.orthax.chebtrim(
                beignet.orthax.cheb2poly(
                    beignet.orthax.chebint(
                        beignet.orthax.poly2cheb(array([0] * i + [1])), m=1, k=[i]
                    )
                ),
                tol=0.000001,
            ),
    ...
    for i in range(5):
        numpy.testing.assert_array_almost_equal(
            beignet.orthax.chebval(
                -1,
                beignet.orthax.chebint(
                    beignet.orthax.poly2cheb(array([0] * i + [1])), m=1, k=[i], lbnd=-1
                ),
            ),
            i,
        )
...
def test_chebline():
    numpy.testing.assert_array_equal(beignet.orthax.chebline(3, 4), array([3, 4]))
...
def test_chebpts2():
    ...
    numpy.testing.assert_array_almost_equal(
        beignet.orthax.chebpts2(3), array([-1, 0, 1])
    )
...
def test_chebval():
    numpy.testing.assert_array_equal(beignet.orthax.chebval(array([]), [1]).size, 0)
...
def test_hermeint():
    ...
    for i in range(2, 5):
        k = [0] * (i - 2) + [1]
        res = beignet.orthax.hermeint(array([0]), m=i, k=k)
        numpy.testing.assert_array_almost_equal(
...
def test_hermeline():
    numpy.testing.assert_array_equal(beignet.orthax.hermeline(3, 4), array([3, 4]))
...
def test_hermeval():
    numpy.testing.assert_array_equal(beignet.orthax.hermeval(array([]), [1]).size, 0)
...
def test_hermint():
    ...
    numpy.testing.assert_raises(
        ValueError, beignet.orthax.hermint, array([0]), 1, [0, 0]
    )
    numpy.testing.assert_raises(
    ...
    for i in range(2, 5):
        ...
        numpy.testing.assert_array_almost_equal(
            beignet.orthax.hermtrim(
                beignet.orthax.hermint(array([0]), m=i, k=k), tol=0.000001
            ),
            [0, 0.5],
        )
...
def test_hermroots():
    numpy.testing.assert_array_almost_equal(beignet.orthax.hermroots([1]), array([]))
    numpy.testing.assert_array_almost_equal(
        beignet.orthax.hermroots([1, 1]), array([-0.5])
    )
    for i in range(2, 5):
...
def test_hermval():
    numpy.testing.assert_equal(beignet.orthax.hermval(array([]), [1]).size, 0)
...
def test_lagint():
    ...
    numpy.testing.assert_raises(
        ValueError, beignet.orthax.lagint, array([0]), 1, [0, 0]
    )
    ...
    for i in range(2, 5):
        ...
        numpy.testing.assert_array_almost_equal(
            beignet.orthax.lagtrim(
                beignet.orthax.lagint(array([0]), m=i, k=k), tol=0.000001
            ),
            [1, -1],
        )
...
def test_lagroots():
    numpy.testing.assert_array_almost_equal(beignet.orthax.lagroots([1]), array([]))
...
def test_lagval():
    numpy.testing.assert_array_equal(beignet.orthax.lagval(array([]), [1]).size, 0)
...
def test_legint():
    ...
    numpy.testing.assert_raises(
        ValueError, beignet.orthax.legint, array([0]), 1, [0, 0]
    )
    ...
    for i in range(2, 5):
        numpy.testing.assert_array_almost_equal(
            beignet.orthax.legtrim(
                beignet.orthax.legint(array([0]), m=i, k=([0] * (i - 2) + [1])),
                tol=0.000001,
            ),
            [0, 1],
        )
...
def test_legline():
    numpy.testing.assert_array_equal(beignet.orthax.legline(3, 4), array([3, 4]))
...
def test_legroots():
    numpy.testing.assert_array_almost_equal(beignet.orthax.legroots([1]), array([]))
    numpy.testing.assert_array_almost_equal(
        beignet.orthax.legroots(array([1, 2])), array([-0.5])
    )
...
def test_legval():
    numpy.testing.assert_array_equal(beignet.orthax.legval(array([]), [1]).size, 0)
...
def test_polycompanion():
    ...
    assert beignet.orthax.polycompanion(array([1, 2]))[0, 0] == -0.5


def test_polyder():
    numpy.testing.assert_raises(TypeError, beignet.orthax.polyder, array([0]), 0.5)
...
def test_polyint():
    ...
    with pytest.raises(ValueError):
        beignet.orthax.polyint(array([0]), 1, [0, 0])
...
def test_polyval():
    numpy.testing.assert_equal(beignet.orthax.polyval(array([]), [1]).size, 0)
...
def test_polyvalfromroots():
    ...
    numpy.testing.assert_equal(
        beignet.orthax.polyvalfromroots(array([]), array([1])).size, 0
    )
    assert beignet.orthax.polyvalfromroots(array([]), array([1])).shape == (0,)

    numpy.testing.assert_equal(
        beignet.orthax.polyvalfromroots(array([]), [[1] * 5]).size, 0
    )

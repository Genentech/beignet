...
from ..functional.statistics import kruskal_wallis_test_power


class KruskalWallisTestPower(Metric):
    r"""
    Compute statistical power for Kruskal-Wallis test.

    This metric calculates the statistical power for Kruskal-Wallis test,
    which is a non-parametric test for comparing multiple independent groups.
    ...
    --------
    >>> import torch
    >>> from beignet.metrics import KruskalWallisTestPower
    >>> metric = KruskalWallisTestPower()
    >>> effect_size = torch.tensor(0.3)
    >>> sample_size = torch.tensor(90)
    ...
    def plot(
        ...
        **kwargs: Any,
        ...
        """Plot Kruskal-Wallis test power visualization."""
        try:
        ...
        if plot_type == "power_curve":
            ...
            for es in effect_sizes:
                power = kruskal_wallis_test_power(
                    torch.tensor(es),
            ...
            if title is None:
                title = f"Kruskal-Wallis Test Power Analysis (α={self.alpha}, N={current_n})"
        else:

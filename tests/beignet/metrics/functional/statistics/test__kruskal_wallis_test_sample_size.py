...
def test_kruskal_wallis_test_sample_size(batch_size, dtype):
    """Test kruskal_wallis_test_sample_size functional wrapper."""
    ...
    try:
        result_functional = (
            beignet.metrics.functional.statistics.kruskal_wallis_test_sample_size(
                test_input,
            )
        )
        result_direct = beignet.statistics.kruskal_wallis_test_sample_size(test_input)
        ...
        assert hasattr(beignet.statistics, "kruskal_wallis_test_sample_size")
        assert callable(
            beignet.metrics.functional.statistics.kruskal_wallis_test_sample_size,
        )
        assert callable(beignet.statistics.kruskal_wallis_test_sample_size)

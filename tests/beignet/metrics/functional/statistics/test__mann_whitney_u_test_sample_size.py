...
def test_mann_whitney_u_test_sample_size(batch_size, dtype):
    """Test mann_whitney_u_test_sample_size functional wrapper."""
    ...
    try:
        result_functional = (
            beignet.metrics.functional.statistics.mann_whitney_u_test_sample_size(
                test_input,
            )
        )
        result_direct = beignet.statistics.mann_whitney_u_test_sample_size(test_input)
        ...
        assert hasattr(beignet.statistics, "mann_whitney_u_test_sample_size")
        assert callable(
            beignet.metrics.functional.statistics.mann_whitney_u_test_sample_size,
        )
        assert callable(beignet.statistics.mann_whitney_u_test_sample_size)
